...
categoria_seleccionada = st.sidebar.selectbox("Categoria de llamada", categorias_disponibles)
...
col3.metric("Satisfaccion Promedio", f"{df_filtrado['score_satisfaccion'].mean():.1f} / 10")
...
fig2 = px.scatter(
    ...
    color="nombre_agente",
    text="nombre_agente",
    labels={"score_qa": "Score QA Promedio", "tasa_error": "Tasa de Error (%)", "nombre_agente": "Agente"}
)
...
volumen_mes = df_filtrado.groupby("mes")["llamadas_por_turno"].sum().reset_index()
...
fig4 = px.imshow(
    ...
    aspect="auto",
    labels={"color": "Score Satisfaccion"},
    text_auto=True
)
...
fig5 = px.box(
    ...
    color="dia_semana",
    labels={"dia_semana": "Dia de la semana", "score_qa": "Score QA"}
)
